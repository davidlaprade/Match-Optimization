...
describe Array, ".finish_assignment" do
	...
	it "does not leave rows without assignments" do
		matrix = [[4, 2, 1, 1, 5, 4, 9, 7, 3, 1, 7],[4, 8, 8, 3, 8, 8, 9, 4, 4, 5, 4],[2, 6, 4, 2, 2, 9, 1, 1, 5, 2, 4],
		[5, 3, 1, 1, 6, 4, 4, 8, 9, 3, 5],[9, 6, 2, 8, 7, 4, 5, 2, 6, 7, 3],[4, 3, 3, 6, 7, 2, 8, 1, 2, 6, 6],
		[9, 9, 1, 8, 2, 2, 5, 3, 8, 7, 5],[6, 7, 4, 5, 2, 6, 2, 3, 4, 1, 5],[8, 8, 3, 3, 7, 6, 1, 2, 1, 3, 2],[3, 9, 9, 7, 2, 3, 4, 2, 3, 3, 2]]
		make_matrix_solveable(matrix)
		assign_needy_zeros(matrix).finish_assignment
		expect(matrix.solution?).to eq(true)
	end

	it "makes sure each row has as many assignments +/- 1 as any other row" do
		matrix = [[3, 6, 7, 1, 6, 5, 2, 6, 2, 4, 4, 1, 7, 8],
		[1, 3, 8, 5, 4, 8, 5, 5, 8, 9, 8, 7, 1, 7],
		[3, 3, 8, 5, 8, 6, 7, 9, 2, 3, 6, 2, 8, 3],
		[2, 3, 6, 6, 3, 9, 4, 8, 9, 9, 7, 1, 8, 4],
		[6, 4, 9, 1, 2, 9, 2, 5, 1, 3, 7, 5, 5, 7]]
		make_matrix_solveable(matrix)
		assign_needy_zeros(matrix).finish_assignment
		expect(matrix.solution?).to eq(true)
	end
	...
	# 40.times do
	# 	it "yeilds produces a solved mask array when called on an array of random size filled with random values" do
	# 		cols = rand(15)+1
	# 		rows = rand(15)+1
	...
	# 		original.print_readable if solution != true
...
describe "make_matrix_solveable" do
	...
	it "should not throw a RuntimeError" do
		matrix = [[1, 6, 6, 3, 2, 8, 9, 8, 2, 2, 8, 5, 4],[9, 5, 1, 3, 5, 1, 1, 8, 2, 8, 7, 6, 1],
		[3, 1, 7, 6, 4, 6, 7, 5, 9, 2, 3, 4, 8],[9, 5, 1, 3, 5, 3, 3, 2, 1, 9, 2, 1, 4],
		[2, 4, 5, 3, 7, 3, 3, 2, 3, 5, 9, 9, 7],[6, 7, 7, 8, 7, 6, 8, 7, 4, 5, 3, 7, 3],
		[5, 6, 5, 5, 8, 5, 4, 6, 8, 2, 5, 8, 6],[3, 1, 9, 5, 6, 9, 2, 7, 1, 1, 5, 3, 6],
		[8, 4, 7, 6, 2, 7, 8, 4, 7, 1, 7, 9, 8]]
		expect(make_matrix_solveable(matrix).solveable?).to eq("true")
	end
...
describe Array, ".make_next_assignment" do
	...
	it "returns 'finished' when called on a solved mask" do
		...
		[2, 4, 1, 3, 6, "!", 5, 4, 6, "X"],[8, "!", 3, 2, 5, "X", 5, 4, 1, 7],[4, 1, 5, 3, 2, 2, "!", 1, "X", 6],
		[6, 6, "X", 8, 4, 4, 8, 7, "!", 3]]
		expect(mask.make_next_assignment).to eq(mask)
	end

	it "does not change the mask it's called on when mask is solved" do
		mask = [[3, 2, 4, "!", 5, 2, 4, 4, 7, 7],["!", 4, 1, 2, 2, "X", 5, 2, 2, 5],[7, 7, "X", 1, 1, 1, 4, 5, 4, "!"],
		[6, 2, "!", 2, 5, 2, 1, 1, 7, 3],["X", 5, "X", 1, "!", 2, 5, 3, 5, "X"],["X", 4, 7, 1, 1, 2, 7, "!", "X", 7],
		[2, 4, 1, 3, 6, "!", 5, 4, 6, "X"],[8, "!", 3, 2, 5, "X", 5, 4, 1, 7],[4, 1, 5, 3, 2, 2, "!", 1, "X", 6],
		[6, 6, "X", 8, 4, 4, 8, 7, "!", 3]]
	...
	it "x's out an unassignable zero after the assignment is made" do
		mask = [["X", 6, "!", "X", 1, 3, 6],[3, 5, 7, 2, 4, "!", 6],[6, 3, 4, 5, 1, "!", 3],["!", 1, 2, 6, 1, 1, 6],
			[3, 6, 5, 4, 1, 6, "!"],[3, 0, 3, 0, 1, 5, 4],[7, "X", "!", "X", 4, 6, "X"],[7, 5, 4, 7, 7, "!", 3],
			[7, 1, 6, "!", 4, 8, 8],[6, "!", 5, 7, 6, 4, 6],[2, "X", "!", "X", 1, "X", 2],["!", "X", 3, 3, "X", 6, 6],
			[1, 1, 1, 3, "!", 2, 4],["!", 2, 2, 5, 1, 6, 7],[6, 0, 2, 7, 2, 6, 0],[3, 6, 2, 0, 0, 8, 0],["!", 2, 5, 3, 5, 2, "X"],
	...
	it "assigns correct zero when a column requires most assignments, and it is the first zero in that column" do
		mask = [["X", 6, "!", "X", 1, 3, 6],[3, 5, 7, 2, 4, "!", 6],[6, 3, 4, 5, 1, "!", 3],["!", 1, 2, 6, 1, 1, 6],
			...
			[6, 1, 3, 6, 6, "!", 1],[4, "!", 3, 8, 1, "X", 7],[7, 0, 5, 4, 0, 1, 3],[5, 2, "!", 1, 4, "X", 7],
			[4, 4, 3, 3, 3, "!", 2],[4, 0, 4, 1, 0, "X", 5],[2, 2, 5, 4, "!", 5, 2],[5, 6, 7, 0, 2, 2, 0]]
		expected_result = [["X", 6, "!", "X", 1, 3, 6],[3, 5, 7, 2, 4, "!", 6],[6, 3, 4, 5, 1, "!", 3],["!", 1, 2, 6, 1, 1, 6],[3, 6, 5, 4, 1, 6, "!"],
			[3, "!", 3, "X", 1, 5, 4],[7, "X", "!", "X", 4, 6, "X"],[7, 5, 4, 7, 7, "!", 3],[7, 1, 6, "!", 4, 8, 8],
			[6, "!", 5, 7, 6, 4, 6],[2, "X", "!", "X", 1, "X", 2],["!", "X", 3, 3, "X", 6, 6],[1, 1, 1, 3, "!", 2, 4],["!", 2, 2, 5, 1, 6, 7],
			[6, 0, 2, 7, 2, 6, 0],[3, 6, 2, 0, 0, 8, 0],["!", 2, 5, 3, 5, 2, "X"],[2, 3, 7, "!", 4, 6, 6],[4, 2, 1, 6, 3, 4, "!"],
			[1, 6, 2, "!", 7, 4, 6],[3, 4, 3, "!", 8, 8, 4],[6, 1, 3, 6, 6, "!", 1],[4, "!", 3, 8, 1, "X", 7],[7, 0, 5, 4, 0, 1, 3],
			[5, 2, "!", 1, 4, "X", 7],[4, 4, 3, 3, 3, "!", 2],[4, 0, 4, 1, 0, "X", 5],[2, 2, 5, 4, "!", 5, 2],[5, 6, 7, 0, 2, 2, 0]]
		mask.make_next_assignment
		expect(mask).to eq(expected_result)
	end
...
describe Array, '.subtract_min_sans_zero_from_rows_to_add_new_column_assignments(submatrix)' do
	...
	it "fixes the matrix minimally when submatrix passed in is only problematic submatrix" do
		...
		expect(matrix).to eq([[0, 0, 1, 0, 5, 6, 9],
							 [5, 7, 1, 0, 1, 0, 23],
							 [0, 0, 3, 9, 1, 0, 6],
							 [1, 6, 1, 0, 1, 8, 2],
							 [0, 3, 0, 4, 0, 7, 0],
	...
	it "fixes the matrix minimally when submatrix passed in is only one of several problematic submatrices" do
		...
		matrix.subtract_min_sans_zero_from_rows_to_add_new_column_assignments(argument)
		expect(matrix).to eq([[1, 0, 1, 0, 5, 6, 9],
							  [6, 7, 1, 0, 1, 0, 23],
							  [1, 0, 3, 9, 1, 0, 6],
							  [2, 6, 1, 0, 1, 8, 1],
							  [8, 3, 9, 0, 2, 7, 0],
	...
	it "fixes the matrix minimally when more than one value has to be changed" do
		...
		matrix.subtract_min_sans_zero_from_rows_to_add_new_column_assignments(argument)
		expect(matrix).to eq( [[0, 6, 4, 0, 1, 6, 9],
							 [1, 4, 4, 0, 3, 9, 23],
							 [6, 0, 0, 0, 0, 9, 6],
							 [1, 3, 4, 0, 3, 8, 4],
							 [7, 0, 6, 0, 4, 7, 4],
...
describe Array, ".make_more_column_assignments_possible" do
	...
	it "works when the smallest problematic submmatrix is all that has to be changed" do
		matrix = [[1,0,1,0,5,6,9],[6,7,1,0,1,0,23],[1,0,3,9,1,0,6],[2,6,1,0,1,8,2],[0,3,0,4,0,7,0],[0,8,0,9,0,9,0],[0,4,0,5,0,4,9]]
		expect(matrix.make_more_column_assignments_possible).to eq( [[0, 0, 1, 0, 5, 6, 9],
																	 [5, 7, 1, 0, 1, 0, 23],
																	 [0, 0, 3, 9, 1, 0, 6],
																	 [1, 6, 1, 0, 1, 8, 2],
																	 [0, 3, 0, 4, 0, 7, 0],
	...
	it "works when two distinct problematic submatrices have to be changed" do
		...
		expect(matrix.make_more_column_assignments_possible).to eq( [[0, 0, 1, 0, 5, 6, 7],
																	 [5, 7, 1, 0, 1, 0, 21],
																	 [0, 0, 3, 9, 1, 0, 4],
																	 [1, 6, 1, 0, 1, 8, 0],
																	 [4, 0, 0, 4, 0, 7, 2],
